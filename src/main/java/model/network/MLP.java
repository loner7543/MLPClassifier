...
public class MLP {
    ...
    private int inputCount;//число входов
    private double[] input;//нейр входного слоя
    private double[] output;//нейр выходного слоя
    private double[] hidden;//нейр скр слоя
    private double[][] inputToHidden;//веса между входным и скрытым слоем
    private double[][] hiddenToOutput;//веса между скр и выходным
    //private SampleType[] sampleTypes;
   private  ActivationFunction activationFunction;
    ...
    public void learn(double[] pattern, double[] answer, int iterationsCount) {
        ...
        do {
            ...
            // считается ошибка - разница между полученным на выходе и ожидаемым
            double[] lError = new double[output.length];
            for (int i = 0; i < output.length; i++) {
                lError[i] = (answer[i] - output[i]) * 0.0;//activationFunction.getDerivativeByFunction(output[i]);
    ...
    public double[] countOuter(double[] input, boolean print) {
        ...
        for (int i = 0; i < hidden.length; i++) {
            ...
            //hidden[i] = activationFunction.getValueByFunction(hidden[i]);
        }
        ...
        for (int i = 0; i < output.length; i++) {
            ...
            //output[i] = activationFunction.getValueByFunction(output[i]);
            if (print) {
